 ...
 * 1.00       24 May 2021     Doug Humberd     Copies Rev Rec date changes from Sales Transactions (SO, Cash Sale, Inv, CM) to Revenue Arrangement
 *
 */
...
function scg_ra_updateRevenueArrangement(type) {
	...
	for (var i=1; itemCount != 0 && i <= itemCount; i++) {
		var revRecStart = newSoRec.getLineItemValue('item', 'custcol_scg_rev_rec_start_date', i);
		var revRecEnd = newSoRec.getLineItemValue('item', 'custcol_scg_rev_rec_end_date', i);
...
function scg_ra_compareDates(oldSoRec, uniqueLineId, revRecStart, revRecEnd) {
  ...
  nlapiLogExecution('DEBUG', 'Line Unique Key', lineNumber);
	if (lineNumber > 0) {
		var oldRevRecStart = oldSoRec.getLineItemValue('item', 'custcol_scg_rev_rec_start_date', lineNumber);
		var oldRevRecEnd = oldSoRec.getLineItemValue('item', 'custcol_scg_rev_rec_end_date', lineNumber);
