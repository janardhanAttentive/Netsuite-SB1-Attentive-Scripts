 ...
 * 1.00				23 Dec 2020		Doug Humberd		Handles client events on the Process Invoices Suitelet
 *
 */
...
function scg_invpdf_updateUrlParams() {
    ...
    subsidiary = (subsidiary) ? '&subsidiary=' + encodeURIComponent(subsidiary) : '';
    ...
    //var urlParams = approvalStartDate + approvalEndDate + tranStartDate + tranEndDate + customer + subsidiary + success + status + failed + email + mail + portal + refresh;
    var urlParams = tranStartDate + tranEndDate + customer + subsidiary + success + status + failed + email + mail + portal + refresh;
...
function scg_invpdf_setFilterChanged(type, name, linenum) {
    if (name == 'custpage_tran_start_date' ||
        //name == 'custpage_approval_start_date' ||
 ...
 * change before allowing any Invoices to be selected for emailing
 *
 * @appliedtorecord invoice
 *
 * @param {String} type Sublist internal id
